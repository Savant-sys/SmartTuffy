...
//We have to modify this prompt for AI
const prompt = `
Hello AI,

I have a database for class schedule.  Refer to the below sample data model... {
        dat: String,
        startTime: String,
        endTime: String,
        roomNumber: String,
...
Please act as a college class schedule assistant and help me with figure out what is asked by a student in the following message:

There below main attributes in the prompt viz. department initials, course number, class name, professor name.

Tell me the values to these if present in the request message. Just reply with Key and values.

Please find that out and fill in the below format and give me the modified response as per the format.

Please use the values for the set of possible values as per below:

Department = [CPSC, EC, CIVIL, MECH]
Day = [Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday]
course number = [101, 102, 103, … 600]
class name is any string value and can start with keyword class.

Please omit the key values if not provided by the student. Just use the available ones in the below format.

FORMAT:
<day>
Department <department initials>
Course <department initials> <course number>
Class <class name>
  ...
  try {
    if (intent) {
      ...
      if (results.length > 0) {
        ...
        return res.status(200).json({ message: `No matching ${type}(s) found for ${value}.` });
      }
    } else {
